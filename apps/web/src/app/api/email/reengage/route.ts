...
import Reengage1, { subject as subject1 } from '@/lib/emails/reengage-1'
import Reengage2, { subject as subject2 } from '@/lib/emails/reengage-2'
import Reengage3, { subject as subject3 } from '@/lib/emails/reengage-3'
...
export async function POST(req: NextRequest) {
  try {
    ...
    let html: string
    if (num === 1) {
      html = await render(Reengage1(props))
    } else if (num === 2) {
      html = await render(Reengage2(props))
    } else {
      html = await render(Reengage3(props))
    }
